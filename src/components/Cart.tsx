...
const Cart: React.FC<CartProps> = ({
    ...
    const handleRemoveItem = (itemId: string, itemName: string) => {
        if (isEditingPending) {
            const confirmed = window.confirm(
                `Remove "${itemName}" from order?\n\nThis item will be marked as REMOVED in the order history.`
            );
            if (confirmed) {
                onRemoveItem(itemId);
            }
    ...
    return (
        <div className="bg-white rounded-lg shadow-lg h-full flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-sm font-bold text-white">
                            {isEditingPending ? '✏️ EDITING ORDER' : 'PRODUCTS'}
                        </h2>
                        <p className="text-xs text-blue-100">
                            {isEditingPending ? `Editing Order #${tokenNumber}` : 'Order Summary'}
                        </p>
                    </div>
                    {/* Cancel Button */}
            ...
            {/* Edit Mode Notice */}
            {isEditingPending && (
                <div className="px-2 pt-2 pb-1 bg-orange-50 border-b border-orange-200 flex-shrink-0">
                    <p className="text-xs text-orange-800 font-semibold">
                        📝 Making changes to this order. Removed items will be tracked.
                    </p>
                </div>
            )}
